...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category_tour = $_POST['category-tour'];
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssiiisiisssssssss",
        $title_tour, 
        $date_tour, 
        $img_tour,
        $price_tour, 
...
<body>
    <?php  
        ...
        $sql_category_tour= "SELECT `name-category-tour` FROM `category-tour`  ORDER BY `name-category-tour` ASC ";
        $result_category_tour = $conn->query($sql_category_tour);
                ...
                <div class="col d-flex flex-column mt-4 row-gap-4">
                    ...
                    <div class="d-flex flex-column row-gap-2">
                        ...
                        <div class="row justify-content-between">
                            ...
                            <div class="col-6">
                                    <label for="category-tour">Loại tour</label>
                                    <select id="category-tour" name="category-tour">
                                        <?php
                                        if ($result_category_tour) {
                                            if($result_category_tour->num_rows > 0) {
                                                while($category_tour = $result_category_tour->fetch_assoc()) {
                                                    echo '<option value="' . htmlspecialchars($category_tour['id-category-tour']) . '">'
                                                    . htmlspecialchars($category_tour['name-category-tour']) . '</option>';
                                                }
                                            }
                                        }
                                        ?>
                        ...
                        <div class="row justify-content-between"> 
                            ...
                            <select id="location-tour" name="location-tour" multiple>
                                <?php
                                if ($result_location) {
                                    if($result_location->num_rows > 0) {
                                        while($location_tour = $result_location->fetch_assoc()) {
                                            echo '<option value="' . htmlspecialchars($location_tour['id-location']) . '">'
