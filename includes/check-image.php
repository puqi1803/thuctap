...
if (isset($_FILES["img-collaborator"]) && $_FILES["img-collaborator"]["error"] == 0) {
    $original_file_name = basename($_FILES["img-collaborator"]["name"]);
    ...
    $check = getimagesize($_FILES["img-collaborator"]["tmp_name"]);
    if ($check !== false) {
        $uploadOk = 1;

        if ($_FILES["img-collaborator"]["size"] > 500000000) {
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($_FILES["img-collaborator"]["tmp_name"], $target_file)) {
