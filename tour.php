...
if ($rq_location) {
    $sql .= " AND `id-location-tour` = '" . $conn->real_escape_string($id_location) . "'";
}
...
if ($rq_location) {
    $sql_count .= " AND `id-location-tour` = '" . $conn->real_escape_string($id_location) . "'";
}
/*if ($rq_start_date) {
...
foreach ($tours as $tour) {
    ...
    $result_name_location = null;
    foreach ($locations as $location) {
        if ($location['id-location'] == $id_location_tour ) {
            $result_name_location = $location['name-location'];
            break;
        }
    } 
}
        ...
        <div class="filter col-3">
            ...
            <div class="filter-content d-flex flex-column mt-4 p-4 row-gap-4 border-round background-gray">
                <div class="filter-criteria">
                    ...
                    echo '<select id="location-tour" name="location-tour" class="w-100 p-2 border-round" >';
                ...
                <?php
                    foreach ($tours as $tour) {
                        echo '<div class="result-content d-flex flex-row border-round">';
                            echo '<img class="tour-img object-fit-cover w-50" src="resources/uploads/' . htmlspecialchars($tour["img-tour"]) . '">';
                            echo '<div class="tour-content d-flex flex-column w-50 row-gap-2 p-4">';
                                echo '<a href="single-tour.php?id-tour=' . htmlspecialchars($tour["id-tour"]) . '">
                                    <h5>' . htmlspecialchars($tour["title-tour"]) . '</h5></a>';
                                    echo '<div class="d-flex flex-wrap column-gap-4 justify-content-between">';
                                        echo '<div class="d-flex flex-row column-gap-2 align-items-center">
                                            ...
                                            <p>Mã chương trình: </p>
                                            <p class="id-tour accent">' . htmlspecialchars($tour["id-tour"]) . '</p>
                                        </div>';
                                echo '</div>';
                                ...
                                echo '<div class="d-flex flex-wrap column-gap-4 justify-content-between">';
                                    echo '<div class="d-flex flex-row column-gap-2 align-items-center">
                                        ...
                                        <p>Khởi hành: </p>
                                        <p class="id-tour accent">' . htmlspecialchars($tour["starting-gate"]) . '</p>
                                    </div>';
                                    echo '<div class="d-flex flex-row column-gap-2 align-items-center">
                                        <i class="icon fa-solid fa-location-dot"></i>
                                        <p>Địa điểm: </p>
                                        <p class="accent">' . htmlspecialchars($result_name_location) . '</p>
                                    </div>';
                                echo '</div>';
                                ...
                                echo '<div class="d-flex flex-wrap column-gap-4 justify-content-between">';
                                    echo '<div class="d-flex flex-row column-gap-2 align-items-center">
                                        ...
                                        <p>Thời gian: </p>
                                        <p class="accent">' . htmlspecialchars($tour["duration-tour"]) . '</p>
                                    </div>';
                                    echo '<div class="d-flex flex-row column-gap-2">
                                        <i class="icon fa-solid fa-calendar-days"></i>
                                        <p>Ngày khởi hành: </p>
                                        <p class="accent">' . formatDate($tour["date-tour"]) . '</p>
                                    </div>';
                                echo '</div>';
                                ...
                                echo '<div class="d-flex flex-row column-gap-4 mt-6 justify-content-between align-items-center">';
                                    echo '<div class="d-flex flex-column">
                                        <p>Giá chỉ từ: </p>
                                        <p class="highlight tour-price">' . number_format($tour["price-tour"]) . '</p>';
                                    echo '</div>';
                                    echo '<div>';
                                        echo '<button class="button-primary px-2 py-2"
                                        onclick="window.open(\'single-tour.php?id-tour=' . htmlspecialchars($tour["id-tour"]) . '\', \'_blank\')">Xem chi tiết</button>';
                                        echo '</div>';
                                echo '</div>';                         
                            echo '</div>';
                        echo '</div>';    
